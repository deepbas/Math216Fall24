...
> Example 2: Building on the information and context from Example 1, design a study where the goal is to calculate the required sample size to achieve a narrower margin of error for the population proportion $p$. Assume a desired margin of error of 0.03 at a $90 \%$ confidence level, maintaining the same population proportion estimate $(\hat{p}=0.65)$. How many individuals should be included in the new sample to meet these precision requirements?

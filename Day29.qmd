...
## Independent Sampling Overview {.font80}
...
- Sample sizes ($n_1 \geq 30$, $n_2 \geq 30$)  
- Normality (for smaller samples)  
- Known variances (or assume equal variance under normality)
:::

## Properties of Sampling Distribution {.font80}

::: cle7
For large samples or small but normally distributed samples:  

- Mean of the distribution of $(\bar{x}_1 - \bar{x}_2)$: $(\mu_1 - \mu_2)$  
...
## Confidence Interval for $(\mu_1 - \mu_2)$ {.font80}

::: cle6

- **Known variances:** $(\bar{x}_1 - \bar{x}_2) \pm z_{\alpha/2} \sqrt{\frac{\sigma_1^2}{n_1} + \frac{\sigma_2^2}{n_2}}$  
- **Unknown variances:** Use pooled standard deviation $s_p = \sqrt{\frac{(n_1-1)s_1^2 + (n_2-1)s_2^2}{n_1+n_2-2}}$:  
...
- Reject $H_0$ if $p$-value < $\alpha$ (commonly 0.05)

...
## Wilcoxon Test {.font80}

::: cle6
The Wilcoxon test is a non-parametric alternative to the t-test, useful when data is not normally distributed. It can be applied in three main contexts:
...
## Wilcoxon Test {.font60}

::: cle6
### One-Sample Test
The one-sample Wilcoxon test is used to assess whether the median of a single sample differs from a known median. It ranks the absolute differences from the hypothesized median, providing insights when assumptions of normality do not hold.

### Paired Samples
The paired-sample Wilcoxon test evaluates the differences between two related samples, such as measurements taken before and after an intervention on the same subjects. It ranks the differences to determine if the median difference is significantly different from zero.

### Two Independent Samples
The two-sample Wilcoxon test (also known as the Mann-Whitney U test) compares the medians of two independent samples. It ranks all the observations from both samples together to test whether one sample tends to have higher values than the other.
...
**zsum.test**: Performs a z-test on summarized data.

```r
zsum.test(mean.x, sigma.x, n.x, mean.y, sigma.y, n.y, alternative = "two.sided", mu = 0, conf.level = 0.95)
```

**tsum.test**: Performs a t-test on summarized data.
